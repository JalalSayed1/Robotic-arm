...
for Ki=0:0.5:2
    i = i+1;
    [tout, xout_Gc] = find_response(Gc, Ki, x, xdot, theta_Fref, Ks, Kg, stepsize, EndTime, comminterval, int_delta_theta);
    % plot:
    Ki_str = num2str(Ki);
    figure(1)
    hold on
...
function [tout,xout] = find_response(Gc, Ki, x, xdot, theta_Fref, Ks, Kg, stepsize, EndTime, comminterval, int_delta_theta)
    % Va is the controller (defined here and at the end of the loop).
    % actuator voltage to drive the motor:
    % Ve = delta theta * Gc + 1/s * (delta theta * Gc * Ki)
    delta_theta = (theta_Fref * Ks) - (x(2) * Ks);
    int_delta_theta = int_delta_theta + stepsize*delta_theta; % x + h*xdot;
    Ve = (delta_theta * Gc) +  Gc * Ki * int_delta_theta;
    Va = Kg * Ve;
    ...
    for time = 0:stepsize:EndTime
        ...
        delta_theta = (theta_Fref * Ks) - (x(2) * Ks);
        int_delta_theta = int_delta_theta + stepsize*delta_theta; % x + h*xdot;
        %integration_term = Gc * Ki * eulerint(delta_theta, stepsize, integration_term);
        Ve = (delta_theta * Gc) +  Gc * Ki * int_delta_theta;
        Va = Kg * Ve;
    end
end
